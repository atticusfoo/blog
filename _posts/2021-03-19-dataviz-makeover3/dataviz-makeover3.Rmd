...
```{r, echo=FALSE, out.width="75%", fig.cap="After Makeover.",layout="l-page", fig.width=14, fig.height=10}
knitr::include_graphics("images/1.1 Madeover.PNG")
```
...
This blogpost will attempt to re-present (makeover) the interactive visualisation as seen in the screenshot above, applying techniques learned in Professor Kam’s class on geo-spatial visualisations. The link to the interactive visualisation can be found [here] (https://public.tableau.com/views/South-eastAsiaArmedConflictAnalysis/South-eastAsiaArmedConflictAnalysis?:language=en&:display_count=y&:origin=viz_share_link). The following sections can be found below:   
